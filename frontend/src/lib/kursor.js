...
export default class Kursor {
  ...
  mousemove() {
    ...
    this.on(container, "mousemove", (event) => {
      ...
      if (!this.hasPointerPosition) {
        this.cursorPosition = { ...this.target };
        this.childPosition = { ...this.target };
  ...
  animate() {
    const cursor = this.kursor || document.querySelector(`.kursor${this.key}`);
    const cursorChild = this.kursorChild || document.querySelector(`.kursorChild${this.key}`);

    if (!cursor || !cursorChild || !this.hasPointerPosition) {
      this.rafId = 0;
      return;
    }

    const moveToward = (position, speed) => {
      position.x += (this.target.x - position.x) * speed;
      position.y += (this.target.y - position.y) * speed;
    };

    moveToward(this.cursorPosition, 0.2);
    moveToward(this.childPosition, 0.55);

    cursor.style.left = `${this.cursorPosition.x}px`;
    cursor.style.top = `${this.cursorPosition.y}px`;
    cursorChild.style.left = `${this.childPosition.x}px`;
    cursorChild.style.top = `${this.childPosition.y}px`;

    const dx = Math.abs(this.target.x - this.cursorPosition.x);
    const dy = Math.abs(this.target.y - this.cursorPosition.y);

    if (dx > 0.1 || dy > 0.1) {
      this.rafId = window.requestAnimationFrame(() => this.animate());
    } else {
      cursor.style.left = `${this.target.x}px`;
      cursor.style.top = `${this.target.y}px`;
      cursorChild.style.left = `${this.target.x}px`;
      cursorChild.style.top = `${this.target.y}px`;
      this.cursorPosition = { ...this.target };
      this.childPosition = { ...this.target };
      this.rafId = 0;
    }
  ...
  hovers() {
    ...
    this.on(document, "mouseover", (event) => {
      if (event.target.closest?.(interactiveSelector)) {
        this.addClass("--hover");
        this.addClass("kursorChild--hover", true);
      }
    });

    this.on(document, "mouseout", (event) => {
      if (event.target.closest?.(interactiveSelector)) {
        this.removeClass("--hover");
        this.removeClass("kursorChild--hover", true);
      }
    });
  }
