...
function SocialLinksEditor({ links = {}, onChange }) {
  ...
  const addLink = () => {
    ...
    onChange({ ...links, [selectedPlatform]: "" });
    setSelectedPlatform("");
    setAdding(false);
  };
  ...
  const updateLink = (key, value) => onChange({ ...links, [key]: value });

  return (
    <div className="space-y-2">
      <label className="text-sm font-medium">Social Media Links</label>
      <div className="space-y-2">
        {SOCIAL_PLATFORMS.filter((p) => links[p.key] !== undefined).map((p) => (
          <div key={p.key} className="flex items-center gap-2">
            <span className="w-24 shrink-0 text-xs font-medium text-muted-foreground">{p.label}</span>
            <input
            ...
            />
            <button type="button" onClick={() => removeLink(p.key)} className="p-1 text-destructive hover:bg-destructive/10 rounded">
              <X size={14} />
            </button>
          </div>
        ))}
        ...
        adding ? (
          <div className="flex items-center gap-2 pt-1">
            ...
            >
              ...
              {available.map((p) => <option key={p.key} value={p.key}>{p.label}</option>)}
            </select>
            <Button type="button" size="sm" onClick={addLink} disabled={!selectedPlatform}>Add</Button>
            <Button type="button" size="sm" variant="ghost" onClick={() => { setAdding(false); setSelectedPlatform(""); }}>
              <X size={14} />
            </Button>
          </div>
        ) : (
...
export function TeamManagement() {
  ...
  const { toast } = useToast();
  const empty = { name: "", role: "", avatar: "", bio: "", social: {} };
  const [editing, setEditing] = useState(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  ...
  const openEdit = (item) => {
    setEditing(item);
    setFormData({ ...empty, ...item, social: item.social || {} });
    setIsModalOpen(true);
  };
    ...
    <AdminLayout>
            ...
            columns={[
              ...
              { key: "role", label: "Role" },
              {
                ...
                render: (value) => {
                  ...
                  return count > 0 ? (
                    <span className="rounded-full bg-primary/10 px-2.5 py-0.5 text-xs font-semibold text-primary">{count} link{count > 1 ? "s" : ""}</span>
          ...
          {selected ? (
            <div className="space-y-1">
              ...
              {selected.social && Object.keys(selected.social).length > 0 && (
                <div className="pt-2">
                  <p className="text-xs text-muted-foreground mb-2 font-medium">Social Links</p>
                  {SOCIAL_PLATFORMS.filter((p) => selected.social?.[p.key]).map((p) => (
                    <div key={p.key} className="flex items-center gap-2 py-1.5 border-b border-border last:border-0">
                      <LinkIcon size={12} className="text-primary shrink-0" />
                      <span className="text-xs text-muted-foreground w-16 shrink-0">{p.label}</span>
                      <a href={selected.social[p.key]} target="_blank" rel="noreferrer" className="text-xs text-primary hover:underline truncate">
                        {selected.social[p.key]}
                      </a>
                    </div>
                  ))}
                </div>
      ...
      {isModalOpen && (
        ...
        >
          ...
          <SocialLinksEditor links={formData.social || {}} onChange={(social) => setFormData((p) => ({ ...p, social }))} />
        </FormModal>
      )}
    </AdminLayout>
  );
